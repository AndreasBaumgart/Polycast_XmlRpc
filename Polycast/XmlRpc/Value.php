 ...
 * called by the Polycast_XmlRpc_Client object and as the return value for those calls.
 *
 * This object as a very important static function Polycast_XmlRpc_Value::getXmlRpcValue, this
 * function acts likes a factory for the Polycast_XmlRpc_Value objects
 *
 * Using this function, users/Polycast_XmlRpc_Client object can create the Polycast_XmlRpc_Value objects
...
abstract class Polycast_XmlRpc_Value
{
     ...
     * of Polycast_XmlRpc_Value objects
     */
    protected $_value;
    ...
    /**
     * Get the native XML-RPC type (the type is one of the Polycast_XmlRpc_Value::XMLRPC_TYPE_* constants)
     *
     * @return string
     */
    ...
    /**
     * Creates a Polycast_XmlRpc_Value* object, representing a native XML-RPC value
     * A XmlRpcValue object can be created in 3 ways:
     * 1. Autodetecting the native type out of a PHP variable
     *    (if $type is not set or equal to Polycast_XmlRpc_Value::AUTO_DETECT_TYPE)
     * 2. By specifing the native type ($type is one of the Polycast_XmlRpc_Value::XMLRPC_TYPE_* constants)
     * 3. From a XML string ($type is set to Polycast_XmlRpc_Value::XML_STRING)
     *
     * By default the value type is autodetected according to it's PHP type
     *
     * @param mixed $value
     * @param Polycast_XmlRpc_Value::constant $type
     *
     * @return Polycast_XmlRpc_Value
     * @static
     */
    public static function getXmlRpcValue($value, $type = self::AUTO_DETECT_TYPE)
    {
        switch ($type) {
            ...
            case self::XMLRPC_TYPE_INTEGER:
                require_once 'Polycast/XmlRpc/Value/Integer.php';
                return new Polycast_XmlRpc_Value_Integer($value);

            case self::XMLRPC_TYPE_DOUBLE:
                require_once 'Polycast/XmlRpc/Value/Double.php';
                return new Polycast_XmlRpc_Value_Double($value);

            case self::XMLRPC_TYPE_BOOLEAN:
                require_once 'Polycast/XmlRpc/Value/Boolean.php';
                return new Polycast_XmlRpc_Value_Boolean($value);

            case self::XMLRPC_TYPE_STRING:
                require_once 'Polycast/XmlRpc/Value/String.php';
                return new Polycast_XmlRpc_Value_String($value);

            case self::XMLRPC_TYPE_BASE64:
                require_once 'Polycast/XmlRpc/Value/Base64.php';
                return new Polycast_XmlRpc_Value_Base64($value);

            case self::XMLRPC_TYPE_NIL:
                require_once 'Polycast/XmlRpc/Value/Nil.php';
                return new Polycast_XmlRpc_Value_Nil();

            case self::XMLRPC_TYPE_DATETIME:
                require_once 'Polycast/XmlRpc/Value/DateTime.php';
                return new Polycast_XmlRpc_Value_DateTime($value);

            case self::XMLRPC_TYPE_ARRAY:
                require_once 'Polycast/XmlRpc/Value/Array.php';
                return new Polycast_XmlRpc_Value_Array($value);

            case self::XMLRPC_TYPE_STRUCT:
                require_once 'Polycast/XmlRpc/Value/Struct.php';
                return new Polycast_XmlRpc_Value_Struct($value);

            default:
                require_once 'Polycast/XmlRpc/Value/Exception.php';
                throw new Polycast_XmlRpc_Value_Exception('Given type is not a '. __CLASS__ .' constant');
        }
    }
    ...
    private static function _phpVarToNativeXmlRpc($value)
    {
        switch (gettype($value)) {
            case 'object':
                ...
                if ($value instanceof Polycast_XmlRpc_Value) {
                    return $value;
                }
            ...
            case 'array':
                ...
                require_once 'Polycast/XmlRpc/Value/Array.php';
                $obj = 'Polycast_XmlRpc_Value_Array';

                // Determine if this is an associative array
                if (!empty($value) && is_array($value) && (array_keys($value) !== range(0, count($value) - 1))) {
                    require_once 'Polycast/XmlRpc/Value/Struct.php';
                    $obj = 'Polycast_XmlRpc_Value_Struct';
                }
    ...
    private static function _xmlStringToNativeXmlRpc($xml)
    {
        if (!$xml instanceof SimpleXMLElement) {
            try {
                ...
                require_once 'Polycast/XmlRpc/Value/Exception.php';
                throw new Polycast_XmlRpc_Value_Exception('Failed to create XML-RPC value from XML string: '.$e->getMessage(),$e->getCode());
            }
        }
        ...
        switch ($type) {
            ...
            case self::XMLRPC_TYPE_INTEGER:
                require_once 'Polycast/XmlRpc/Value/Integer.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Integer($value);
                break;
            case self::XMLRPC_TYPE_DOUBLE:
                require_once 'Polycast/XmlRpc/Value/Double.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Double($value);
                break;
            case self::XMLRPC_TYPE_BOOLEAN:
                require_once 'Polycast/XmlRpc/Value/Boolean.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Boolean($value);
                break;
            case self::XMLRPC_TYPE_STRING:
                require_once 'Polycast/XmlRpc/Value/String.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_String($value);
                break;
            case self::XMLRPC_TYPE_DATETIME:  // The value should already be in a iso8601 format
                require_once 'Polycast/XmlRpc/Value/DateTime.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_DateTime($value);
                break;
            case self::XMLRPC_TYPE_BASE64:    // The value should already be base64 encoded
                require_once 'Polycast/XmlRpc/Value/Base64.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Base64($value ,true);
                break;
            case self::XMLRPC_TYPE_NIL:    // The value should always be NULL
                require_once 'Polycast/XmlRpc/Value/Nil.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Nil();
                break;
            case self::XMLRPC_TYPE_ARRAY:
                ...
                if (null === $data) {
                    require_once 'Polycast/XmlRpc/Value/Exception.php';
                    throw new Polycast_XmlRpc_Value_Exception('Invalid XML for XML-RPC native '. self::XMLRPC_TYPE_ARRAY .' type: ARRAY tag must contain DATA tag');
                }
                $values = array();
                // Parse all the elements of the array from the XML string
                // (simple xml element) to Polycast_XmlRpc_Value objects
                foreach ($data->value as $element) {
                    $values[] = self::_xmlStringToNativeXmlRpc($element);
                }
                require_once 'Polycast/XmlRpc/Value/Array.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Array($values);
                break;
            case self::XMLRPC_TYPE_STRUCT:
                $values = array();
                ...
                foreach ($value->member as $member) {
                    ...
                    if (!isset($member->value) or !isset($member->name)) {
                        continue;
                        //throw new Polycast_XmlRpc_Value_Exception('Member of the '. self::XMLRPC_TYPE_STRUCT .' XML-RPC native type must contain a VALUE tag');
                    }
                    $values[(string)$member->name] = self::_xmlStringToNativeXmlRpc($member->value);
                }
                require_once 'Polycast/XmlRpc/Value/Struct.php';
                $xmlrpc_val = new Polycast_XmlRpc_Value_Struct($values);
                break;
            default:
                require_once 'Polycast/XmlRpc/Value/Exception.php';
                throw new Polycast_XmlRpc_Value_Exception('Value type \''. $type .'\' parsed from the XML string is not a known XML-RPC native type');
                break;
        }
