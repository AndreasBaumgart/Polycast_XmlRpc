...
class Polycast_XmlRpc_Request
{
    ...
    public function setMethod($method)
    {
        if (!is_string($method) || !preg_match('/^[a-z0-9_.:\/]+$/i', $method)) {
            $this->_fault = new Polycast_XmlRpc_Fault(634, 'Invalid method name ("' . $method . '")');
    ...
    public function addParam($value, $type = null)
    {
        ...
        if (null === $type) {
            ...
            if ($value instanceof Polycast_XmlRpc_Value) {
                ...
                $xmlRpcValue = Polycast_XmlRpc_Value::getXmlRpcValue($value);
    ...
    public function setParams()
    {
        ...
        if ((1 == $argc) && is_array($argv[0])) {
            ...
            foreach ($argv[0] as $arg) {
                ...
                if (!isset($arg['type'])) {
                    $xmlRpcValue = Polycast_XmlRpc_Value::getXmlRpcValue($arg['value']);
            ...
            if ($wellFormed) {
                ...
                $xmlRpcParams  = array();
                foreach ($argv[0] as $arg) {
                    if ($arg instanceof Polycast_XmlRpc_Value) {
                        $type = $arg->getType();
                    } else {
                        $xmlRpcValue = Polycast_XmlRpc_Value::getXmlRpcValue($arg);
                        $type        = $xmlRpcValue->getType();
                    }
    ...
    public function loadXml($request)
    {
        if (!is_string($request)) {
            $this->_fault = new Polycast_XmlRpc_Fault(635);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        try {
            ...
            // Not valid XML
            $this->_fault = new Polycast_XmlRpc_Fault(631);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        if (empty($xml->methodName)) {
            // Missing method name
            $this->_fault = new Polycast_XmlRpc_Fault(632);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        if (!empty($xml->params)) {
            ...
            foreach ($xml->params->children() as $param) {
                if (!isset($param->value)) {
                    $this->_fault = new Polycast_XmlRpc_Fault(633);
                    $this->_fault->setEncoding($this->getEncoding());
                    return false;
                }

                try {
                    $param   = Polycast_XmlRpc_Value::getXmlRpcValue($param->value, Polycast_XmlRpc_Value::XML_STRING);
                    $types[] = $param->getType();
                    $argv[]  = $param->getValue();
                } catch (Exception $e) {
                    $this->_fault = new Polycast_XmlRpc_Fault(636);
                    $this->_fault->setEncoding($this->getEncoding());
                    return false;
                }
            }
    ...
    {
        return $this->_fault instanceof Polycast_XmlRpc_Fault;
    }

    /**
     * Retrieve the fault response, if any
     *
     * @return null|Polycast_XmlRpc_Fault
     */
    ...
    protected function _getXmlRpcParams()
    {
        ...
        if (is_array($this->_xmlRpcParams)) {
            foreach ($this->_xmlRpcParams as $param) {
                $value = $param['value'];
                $type  = isset($param['type']) ? $param['type'] : Polycast_XmlRpc_Value::AUTO_DETECT_TYPE;

                if (!$value instanceof Polycast_XmlRpc_Value) {
                    $value = Polycast_XmlRpc_Value::getXmlRpcValue($value, $type);
                }
                $params[] = $value;
            }
        }
