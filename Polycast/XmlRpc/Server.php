 ...
 */
require_once 'Polycast/XmlRpc/Response/Http.php';

/**
 * XMLRPC server fault class
 */
require_once 'Polycast/XmlRpc/Server/Fault.php';

/**
 * XMLRPC server system methods class
 */
require_once 'Polycast/XmlRpc/Server/System.php';
 ...
 * Example:
 * <code>
 * require_once 'Polycast/XmlRpc/Server.php';
 * require_once 'Polycast/XmlRpc/Server/Cache.php';
 * require_once 'Polycast/XmlRpc/Server/Fault.php';
 * require_once 'My/Exception.php';
 * require_once 'My/Fault/Observer.php';
 *
 * // Instantiate server
 * $server = new Polycast_XmlRpc_Server();
 *
 * // Allow some exceptions to report as fault responses:
 * Polycast_XmlRpc_Server_Fault::attachFaultException('My_Exception');
 * Polycast_XmlRpc_Server_Fault::attachObserver('My_Fault_Observer');
 *
 * // Get or build dispatch table:
 * if (!Polycast_XmlRpc_Server_Cache::get($filename, $server)) {
 ...
 *     Polycast_XmlRpc_Server_Cache::save($filename, $server);
 * }
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Polycast_XmlRpc_Server extends Zend_Server_Abstract
{
    /**
     * Character encoding
     * @var string
    ...
    /**
     * Request processed
     * @var null|Polycast_XmlRpc_Request
     */
    protected $_request = null;

    /**
     * Class to use for responses; defaults to {@link Polycast_XmlRpc_Response_Http}
     * @var string
     */
    protected $_responseClass = 'Polycast_XmlRpc_Response_Http';
     ...
     * @param  array $params
     * @return mixed
     * @throws Polycast_XmlRpc_Server_Exception
     */
    public function __call($method, $params)
    {
        ...
        if (!method_exists($system, $method)) {
            ...
            throw new Polycast_XmlRpc_Server_Exception('Unknown instance method called on server: ' . $method);
        }
    ...
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && !is_array($function)) {
            require_once 'Polycast/XmlRpc/Server/Exception.php';
            throw new Polycast_XmlRpc_Server_Exception('Unable to attach function; invalid', 611);
        }
     ...
     * @return void
     * @throws Polycast_XmlRpc_Server_Exception on invalid input
     */
    public function setClass($class, $namespace = '', $argv = null)
    {
        if (is_string($class) && !class_exists($class)) {
            require_once 'Polycast/XmlRpc/Server/Exception.php';
            throw new Polycast_XmlRpc_Server_Exception('Invalid method class', 610);
        }
    ...
    public function fault($fault = null, $code = 404)
    {
        if (!$fault instanceof Exception) {
            ...
            require_once 'Polycast/XmlRpc/Server/Exception.php';
            $fault = new Polycast_XmlRpc_Server_Exception($fault, $code);
        }

        return Polycast_XmlRpc_Server_Fault::getInstance($fault);
    }

    /**
     * Handle an xmlrpc call
     *
     * @param Polycast_XmlRpc_Request $request Optional
     * @return Polycast_XmlRpc_Response|Polycast_XmlRpc_Fault
     */
    public function handle($request = false)
    {
        // Get request
        if ((!$request || !$request instanceof Polycast_XmlRpc_Request)
            && (null === ($request = $this->getRequest()))
        ) {
            require_once 'Polycast/XmlRpc/Request/Http.php';
            $request = new Polycast_XmlRpc_Request_Http();
     ...
     * results pulled from {@link Polycast_XmlRpc_Server_Cache::get()}.
     *
    ...
    public function loadFunctions($definition)
    {
        if (!is_array($definition) && (!$definition instanceof Zend_Server_Definition)) {
            ...
            require_once 'Polycast/XmlRpc/Server/Exception.php';
            throw new Polycast_XmlRpc_Server_Exception('Unable to load server definition; must be an array or Zend_Server_Definition, received ' . $type, 612);
        }
    ...
    /**
     * Do nothing; persistence is handled via {@link Polycast_XmlRpc_Server_Cache}
     *
     * @param  mixed $mode
     * @return void
     */
     ...
     *
     * @param string|Polycast_XmlRpc_Request $request
     * @return Polycast_XmlRpc_Server
     * @throws Polycast_XmlRpc_Server_Exception on invalid request class or object
     */
    public function setRequest($request)
    {
        if (is_string($request) && class_exists($request)) {
            $request = new $request();
            if (!$request instanceof Polycast_XmlRpc_Request) {
                require_once 'Polycast/XmlRpc/Server/Exception.php';
                throw new Polycast_XmlRpc_Server_Exception('Invalid request class');
            }
            $request->setEncoding($this->getEncoding());
        } elseif (!$request instanceof Polycast_XmlRpc_Request) {
            require_once 'Polycast/XmlRpc/Server/Exception.php';
            throw new Polycast_XmlRpc_Server_Exception('Invalid request object');
        }
     ...
     *
     * @return null|Polycast_XmlRpc_Request
     */
    ...
    public function setResponseClass($class)
    {
        if (!class_exists($class) or
            ($c = new ReflectionClass($class) and !$c->isSubclassOf('Polycast_XmlRpc_Response'))) {

            require_once 'Polycast/XmlRpc/Server/Exception.php';
            throw new Polycast_XmlRpc_Server_Exception('Invalid response class');
        }
     ...
     *
     * @return Polycast_XmlRpc_Server_System
     */
    ...
    {
        $system = new Polycast_XmlRpc_Server_System($this);
        $this->_system = $system;
