 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Polycast_XmlRpc_Value_DateTime extends Polycast_XmlRpc_Value_Scalar
{
    /**
    ...
    public function __construct($value)
    {
        ...
        if (is_numeric($value)) {   // The value is numeric, we make sure it is an integer
            ...
            if ($timestamp === false || $timestamp == -1) { // cannot convert the value to a timestamp
                throw new Polycast_XmlRpc_Value_Exception('Cannot convert given value \''. $value .'\' to a timestamp');
            }
        }
