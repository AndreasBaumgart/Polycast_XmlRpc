 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Polycast_XmlRpc_Server_System
{
    /**
     * @var Polycast_XmlRpc_Server
     */
    protected $_server;
     ...
     * Constructor
     * 
     * @param  Polycast_XmlRpc_Server $server 
     * @return void
     */
    public function __construct(Polycast_XmlRpc_Server $server)
    {
        $this->_server = $server;
    }
    ...
    public function methodHelp($method)
    {
        ...
        if (!$table->hasMethod($method)) {
            ...
            throw new Polycast_XmlRpc_Server_Exception('Method "' . $method . '" does not exist', 640);
        }
        ...
        foreach ($methods as $method) {
            ...
            if (!$fault) {
                try {
                    ...
                    if ($response instanceof Polycast_XmlRpc_Fault 
                        || $response->isFault()
                    ) {
                        $fault = $response;
                    } else {
