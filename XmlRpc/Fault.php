 ...
 * to instantiate a Polycast_XmlRpc_Server_Fault.
 *
 * @category   Zend
 * @package    Zend_XmlRpc
 * @copyright  Copyright (c) 2005-2009 Zend Technologies USA Inc. (http://www.zend.com)
...
class Polycast_XmlRpc_Fault
{
    ...
    {
        if (!is_string($fault)) {
            require_once 'Polycast/XmlRpc/Exception.php';
            throw new Polycast_XmlRpc_Exception('Invalid XML provided to fault');
        }
        ...
        try {
            ...
            // Not valid XML
            require_once 'Polycast/XmlRpc/Exception.php';
            throw new Polycast_XmlRpc_Exception('Failed to parse XML fault: ' .  $e->getMessage(), 500);
        }
        ...
        if (!$xml->fault->value->struct) {
            // not a proper fault
            require_once 'Polycast/XmlRpc/Exception.php';
            throw new Polycast_XmlRpc_Exception('Invalid fault structure', 500);
        }

        $structXml = $xml->fault->value->asXML();
        $structXml = preg_replace('/<\?xml version=.*?\?>/i', '', $structXml);
        $struct    = Polycast_XmlRpc_Value::getXmlRpcValue(trim($structXml), Polycast_XmlRpc_Value::XML_STRING);
        ...
        if (empty($code) && empty($message)) {
            require_once 'Polycast/XmlRpc/Exception.php';
            throw new Polycast_XmlRpc_Exception('Fault code and string required');
        }
